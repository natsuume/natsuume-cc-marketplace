...
- **失敗検知の対称性**: 3 軸とも `tool_response.is_error` / `interrupted` の同じ判定経路で silent-pass を防げます。v2.x までは Skill / Bash / Agent の 3 通りの失敗検知ロジックが並立していました。
- **`auto-mark.sh` の簡略化**: PRECHECK_RE / case 文は subagent_type 末尾一致 (`code-reviewer` / `security-reviewer`) のみを検知する形に縮みました。Skill 検知は全廃。

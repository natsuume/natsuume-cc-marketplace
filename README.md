...
| [codex-advisor](#codex-advisor) | 0.1.0 | Anthropic の Advisor tool パターンを移植し、OpenAI Codex を助言役 (advisor) として利用するプラグイン。SessionStart で相談タイミング規律 (実質作業前・完了宣言前・行き詰まり・方針転換) を常時注入し、`/codex-advisor:consult` skill で Codex に read-only 相談 (effort xhigh 固定) して plan / course-correction の助言を受け取る。要 openai-codex plugin + Codex CLI |

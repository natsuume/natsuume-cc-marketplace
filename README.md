...
| プラグイン | バージョン | 説明 |
|-----------|------------:|------|
| [git-guardrails](#git-guardrails) | 0.6.7 | GitHub Flow を構造強制するプラグイン。デフォルトブランチ (master/main) への直接書き込み経路 (commit / push / master/main を head とする PR 作成) を PreToolUse hook で deny し、変更を GitHub 上の PR merge 経由のみに限定する |
...
| [agent-discipline](#agent-discipline) | 0.28.0 | 作業規律を SessionStart / SubagentStart prompt で配送し、gh issue/pr body の未決定事項を PreToolUse で検知するプラグイン |
| [experimental-agent-discipline](#experimental-agent-discipline) | 0.4.0 | agent-discipline の実験的 fork。Fable 週次枠の使用率が閾値 (既定 50%) 以下のあいだ、effort low 固定の専用 agent への Fable subagent 委任を許可する (agent-discipline と切替運用) |

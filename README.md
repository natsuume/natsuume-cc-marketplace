...
| [natsuume-writing](#natsuume-writing) | 0.1.0 | テックブログ・技術書執筆を支援するプラグイン。natsuume の過去執筆物から抽象化した執筆ルール (文体コア + 媒体プロファイル) を配置し、SessionStart でコア要点を常時注入する。outline / draft / review skill は今後追加予定 (#208 / #209 / #210)。執筆プロジェクトでのみ enable して使う |
...
現時点では rules 配置 + SessionStart コア注入 hook のみを提供します。outline / draft / review の各 skill は #208 / #209 / #210 で追加予定です。
...
`writing` `tech-blog` `technical-writing` `style-guide` `system-prompt` `hook`

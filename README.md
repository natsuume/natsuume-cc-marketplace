...
| プラグイン | バージョン | 説明 |
|-----------|-----------|------|
| [git-guardrails](#git-guardrails) | 0.4.0 | GitHub Flow に準拠した Git ワークフロー。デフォルトブランチへの直接書き込み経路 (commit / push / PR head) をすべて deny し、変更は GitHub 上の PR merge 経由のみで取り込む。rebase ワークフロー Skill も提供。v0.4.0 で quote/heredoc 内のコマンド例文への誤 deny を修正 (segment/token ベース検出へ移行) |
| [enforce-draft-pr](#enforce-draft-pr) | 0.2.2 | `gh pr create` に `--draft` を自動付与する PreToolUse フックプラグイン (任意導入)。v0.2.1 で env-skip ループ境界越境による重複 `--draft` 挿入 parser bug を修正 |
...
| [fable-discipline](#fable-discipline) | 0.3.0 | Fable (Mythos 級モデル) をメインセッションで使う際の分業規律プラグイン。SessionStart で「Fable は曖昧さの分解・他モデル向け指示書作成・全体設計検討・検収に徹し、実装と具体調査は Sonnet/Opus のサブエージェントへ委任する」誘導層をハイブリッド方式 (model 判定可 = 無条件文 / 欠落 = 自己ゲート文) で注入し、PreToolUse (`Agent\|Task`) で env の fable 強制・fable 明示指定・「model 未指定 + fable セッション継承」の 3 経路を deny する防波堤層 (判定順序はモデル解決順序 env > 明示指定 > 継承と一致) を提供。主防御は `CLAUDE_CODE_SUBAGENT_MODEL` env (明示指定・frontmatter・Workflow 内部 `agent()` より優先されることを実測検証済み) で、hook は env 誤設定・撤去時の defense-in-depth。v0.2.0 で委任指示の必須要素 (3 面 + default-deny 安全弁 + 終了時自己点検) とエスカレーションフロー (第三の正規終了) を誘導層に追加。v0.3.0 で誘導層・deny メッセージを Fable 5 / Sonnet 5 公式プロンプトガイドに整合 (並列・非同期委任 / fresh-context verifier 検収 / effort 明示 / coverage-first 委任 / 受領時 grounding 等) |

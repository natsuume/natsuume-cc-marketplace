...
| [natsuume-writing](#natsuume-writing) | 0.2.0 | テックブログ・技術書執筆を支援するプラグイン。natsuume の過去執筆物から抽象化した執筆ルール (文体コア + 媒体プロファイル) を配置し、SessionStart でコア要点を常時注入する。outline skill (章立ての壁打ち + インファイルスケルトン書き込み) を提供。draft / review skill は今後追加予定 (#209 / #210)。執筆プロジェクトでのみ enable して使う |
...
現時点では rules 配置 + SessionStart コア注入 hook + outline skill (章立ての壁打ち + インファイルスケルトン書き込み) を提供します。draft / review の各 skill は #209 / #210 で追加予定です。
...
| スキル名 | コマンド | 説明 |
|---------|---------|------|
| outline | `/outline` | 壁打ちで技術記事・技術書の章立て・セクション構成を決め、記事ファイルにインファイルスケルトン (見出し + HTML コメント) を書き込む |

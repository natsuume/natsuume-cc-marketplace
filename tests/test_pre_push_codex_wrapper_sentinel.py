...
(`<git-dir>/pre-push-codex-review-terminal-<run id>`) の絶対パスと run id を案内行
...
class WrapperTerminalSentinelTest(unittest.TestCase):
    ...
    def sentinels(self, repository: Path) -> list[Path]:
        return sorted((repository / ".git").glob(f"{SENTINEL_NAME_PREFIX}-*"))
    ...
    def test_each_run_writes_its_own_sentinel(self) -> None:
        ...
        self.assertEqual(
            self.sentinels(repository),
            sorted(
                (
                    self.sentinel_path(repository, first),
                    self.sentinel_path(repository, second),
                )
            ),
        )

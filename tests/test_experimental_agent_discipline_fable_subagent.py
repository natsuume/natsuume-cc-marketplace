...
- version 整合 (``PluginVersionConsistencyTest``): plugin.json / marketplace.json /
  リポジトリ直下 README / plugin README の 4 箇所が name と v0.6.0 で一致すること。
...
class PluginVersionConsistencyTest(unittest.TestCase):
    ...
    def test_plugin_json_declares_experimental_name_and_version(self) -> None:
        """plugin.json の name と version が experimental-agent-discipline / 0.6.0 である。"""
    ...
    def test_plugin_readme_version_heading_declares_the_version(self) -> None:
        """plugin README の `## バージョン` 直下の行が v0.6.0 である。"""

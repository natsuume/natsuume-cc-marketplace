...
- fork 元 (agent-discipline v0.26.0) の更新の取り込みは手動です。agent-discipline を更新したときは、差分ファイル以外を fork へ反映する作業が別途必要です。
...
- **`CLAUDE_CODE_SUBAGENT_MODEL_FORCE` が設定されていないこと**。FORCE が有効な間は `CLAUDE_CODE_SUBAGENT_MODEL` (未設定ならメインセッションのモデル) が全サブエージェントの実効モデルになり、agent 定義 frontmatter の `model: fable` が無視されるため、専用 agent が Fable 以外のモデルで effort low のまま実行されてしまいます。そのため、FORCE が有効な環境では専用 agent への Fable 委任は deny されます。Fable 委任を使うときは FORCE を解除してください。`CLAUDE_CODE_SUBAGENT_MODEL` 自体は、FORCE が無効なら明示した `model: "fable"` が優先されるため設定されていても構いません (env が fable を指す場合は、model を明示しない継承経路が deny されます)。
...
| deny 理由 | 対処 |
|---|---|
| `CLAUDE_CODE_SUBAGENT_MODEL_FORCE` が有効になっている | FORCE を解除して再実行する。env はセッションを超える設定のため、解除はユーザに依頼する。急ぐ場合は sonnet / opus へ通常委任する |
| `CLAUDE_CODE_SUBAGENT_MODEL` が fable を指していて model を明示していない | `model` に sonnet / opus (機械的作業なら haiku) を明示して再実行する。env 自体を直す場合はセッションを超える設定のためユーザに依頼する |
| fork の継承元 (メインセッション) が Fable | fork をやめ、必要な文脈を指示文に埋め込んだ新規起動に切り替えて model を明示する |
...
Claude Code のモデル解決順序は 明示 `model` > agent 定義の frontmatter > `CLAUDE_CODE_SUBAGENT_MODEL` > メインセッション継承 で、`CLAUDE_CODE_SUBAGENT_MODEL_FORCE` (`1` / `true`) が設定されている場合のみ env (未設定ならメインセッションのモデル) が全てを上書きします。`subagent_type` が `fork` のサブエージェントは model 指定にも env にも依らずメインセッションのモデルを継承します。

1. `subagent_type` が `fork` → メインセッションのモデルで判定する。fable なら deny
2. `CLAUDE_CODE_SUBAGENT_MODEL_FORCE` が有効 → 実効モデルは `CLAUDE_CODE_SUBAGENT_MODEL` (未設定ならメインセッションのモデル)
   - 実効モデルが fable → deny (model を明示しても変わらないため、FORCE の解除または env の修正を案内)
   - 実効モデルが非 fable でも、専用 agent への委任なら deny (frontmatter の `model: fable` が無視され、専用 agent が Fable 以外のモデルで effort low のまま実行されるため。FORCE の解除を案内)
   - それ以外 → allow
3. `tool_input.model` に fable が指定されている
...
5. `tool_input.model` 未指定 (継承経路)
   - a. `subagent_type` が専用 agent 2 種を指す → deny (`model: "fable"` の明示を案内)
   - b. それ以外 → `CLAUDE_CODE_SUBAGENT_MODEL` が非空なら、fable を指すとき deny・それ以外は allow。env 不在で subagent 内 (hook 入力に `agent_id` あり) からの起動なら deny (継承先が起動元 subagent のモデルになり、専用 agent から Fable を継承しうるため)。env 不在ならセッションの model state が fable のとき deny、state 不明かつ pending マーカーがあるとき deny、それ以外は allow
...
- この判定は Agent / Task tool による**起動時の入場判定**です。起動後の継続 (SendMessage による同一 agent の再開) と Workflow tool 内部の `agent()` 呼び出しは PreToolUse `Agent|Task` で観測できないため再判定しません。起動時に閾値以下だった専用 agent が動作中に閾値を超えても停止しません

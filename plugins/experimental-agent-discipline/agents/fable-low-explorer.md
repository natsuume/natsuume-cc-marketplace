...
- `CLAUDE_CODE_SUBAGENT_MODEL_FORCE` が設定されている環境では、`CLAUDE_CODE_SUBAGENT_MODEL` (未設定ならメインセッションのモデル) が明示した model と frontmatter の `model: fable` を上書きし、Fable 以外のモデルが effort low で実行されるため、PreToolUse hook が deny します。FORCE を解除してから起動するか、sonnet / opus への通常委任に切り替えます。

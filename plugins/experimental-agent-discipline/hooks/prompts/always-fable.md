<!--
  agent-discipline: 常時適用ルール (FABLE 版)
  ...
  ルール ID セットは、3 ファイル構成の always-sonnet-{1,2,3}.md の ID セットの和集合と
  完全一致すること (lint-prompt-sync.sh チェック 1 が検証する)。
-->
...
**境界**: 判断材料の説明テキストと `AskUserQuestion` は同一ターンで接続し、テキストのみで turn を終えて返信を待ってから質問する分割をしない。説明テキストがあっても質問文・選択肢は自己完結に書く。説明テキストが見えない事象が起きた場合は分割手順に戻さず、ユーザに報告して確認を取る。

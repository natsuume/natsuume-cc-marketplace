...
- `cd /other && git commit` のように同一コマンド内で `cd` の後に `git commit` を実行する形式も、本フックは `cd` 後の cwd を認識できず対象 repo がズレるため、上記の repo override と同様に **fail closed (deny) でブロック** します。判定は `cd` より後にある commit だけが対象で、`git commit -m x && cd /tmp && npm test` のように `cd` より前にある commit は cwd repo を対象として lint します。`cd` の後でも `git commit --dry-run` / `--help` のように commit を作成しない呼び出しや、`cd docs && grep "git commit" .` のように commit を実行しないコマンドは deny しません。対象 repo に `cd` してから別の Bash 呼び出しで commit してください。

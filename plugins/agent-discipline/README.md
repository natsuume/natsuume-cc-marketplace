...
- **Step 0 を「先頭 literal 一致」 から「command 内 literal contains」 に緩和**: codex review 5th round の P2 指摘 (`cd repo && gh issue create ...` / `cat > body.md && gh issue create -F body.md` のような compound command は `cd dir && cmd` の section 1 例外として allowed だが、 Step 0 の strict head match では silent pass する) への対処。 Step 0 を compound command 内に target subcommand を持つケースも catch するよう「command 内のどこかに target literal が含まれるか」 ベースに緩和。 第一の narrow scope は依然 `if` field の hook config 段階で確保しており、 Step 0 は defense-in-depth として `if` の fall through / compound 経路の両方を catch する

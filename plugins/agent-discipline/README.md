...
- **gh CLI 公式 short alias `-b` / `-F` を Step 1 enumeration に追加**: code-reviewer 3rd round で sub-threshold (confidence 50-60%) 指摘された short flag 漏れ (`gh issue create -b "..."` / `gh issue create -F body.md` 形式が prompt Step 1 enumeration の long flag だけの list では「指定なし → editor 経路 → ok:true」 で silent pass する可能性) への対処。 gh CLI 公式仕様で `-b = --body` / `-F = --body-file` のため LLM 一般化に期待せず明示し、 fail-closed 原則を厳密化

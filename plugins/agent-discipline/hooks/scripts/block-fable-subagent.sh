...
# 場合に限り許可する (用途 = advisor への限定は分業規律の prompt が担い、
...
FABLE_FALLBACK_GUIDE="fable-advisor-runner は再起動せずスキップしてください。それ以外の委任では Fable を使わず、model に非 Fable のモデル (例: \`model: \"opus\"\`) を明示してください。"

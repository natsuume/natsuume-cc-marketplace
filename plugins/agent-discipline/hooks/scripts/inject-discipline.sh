...
#     discipline-fable.md) を注入、マーカーを `final` にする
#   - pending 無し + state が非 fable → 分業規律 sonnet 版 (見出し + discipline-sonnet.md) を
#     注入、マーカーを `final` にする
...
#     本要素を優先する旨を前置きに含める)
#   - pending 無し + state が非 fable に確定していた → 注入なしでマーカーを `final` に更新する
#     (配送済みの Sonnet 版がそのまま確定内容であるため)
...
#   → 無音 exit 0、マーカーは書かない (次プロンプトで再試行)
...
$FABLE_BODY"
  ...
  # 非 fable に確定: 自己ゲート付きで配送済みの Sonnet 版がそのまま確定内容のため、

...
5. **作業 branch 作成 + 即 push**: `git switch -c <branch>` → `git commit --allow-empty -m "wip: claim issue #<N> session=<セッションID>"` → `git push -u origin <branch>`。push 失敗 (同名 branch 既存) なら撤退、成功なら独占権確定。commit message への session 埋込は、同一内容の空 commit が同一 OID になり後発 push が already up to date で成功扱いになる経路を塞ぐ
...
`ai:in-progress` ラベルは対応する PR が merge された時のみ削除する。着手中断 / 撤退時はラベルを残し、claim comment と branch のみ削除する。claim comment 本文の `session=` 値が自分のセッション ID と一致するかで「自分の claim か」を判定し (`session=` キーの無い旧形式 claim は自分のものと確認できないため他 session 扱い)、他 session の claim / branch / ラベルは絶対に削除しない。

<!--
  agent-discipline: 常時適用ルール (SONNET 版、part 2/3、issue #236)
  issue #236 (注入ペイロード分割) による always-sonnet.md 3 分割の 2 番目。rule 本文・
  rule ID マーカーは分割前から一字一句無変更。ヘッダコメント・part 表記・part 冒頭の説明文
  のみが分割に伴う追加。ルール ID セットの完全一致の受入基準は part 1/3 (always-sonnet-1.md)
  のヘッダコメントを参照。
-->

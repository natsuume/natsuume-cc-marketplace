...
受入基準を書く際は、I/O 契約の各入力・状態の定義域の端から境界・異常系の状況を列挙し、それぞれの挙動を受入基準に明記します (例: 空・0 件・枯渇・上限・重複・不正値。この例示は網羅ではなく導出の起点であり、例示に無い境界も I/O 契約の各入力・状態の定義域から同様に導いてください)。挙動が一意に決まらないものは `AskUserQuestion` で確定します。理由: 境界での挙動は複数の設計が成立する「決定」であり (`rule:design-approval` の対象)、未記載のまま起票すると実装セッションでエスカレーションを要する仕様穴になります。

一方、確定した挙動をテストケースへ展開する作業 (ケースの網羅列挙・fixture・検証方法の指定) は issue には書かず、実装時の spec-first 2 段階の Phase A (`rule:tdd-two-phase`) に委ねます。この禁止は検証手段への展開のみを対象とし、テストすべき挙動の観点を受入基準に列挙すること自体は妨げません。
...
| 受入基準 | 完了と判定できる具体的な条件 (曖昧な形容詞を避ける)。境界・異常系の挙動を含める |

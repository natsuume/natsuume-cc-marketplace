#!/bin/bash
# exit-trap.sh
# pre-push-codex-review プラグインの 3 つの hook script (block-pre-push-codex.sh /
# auto-mark.sh / block-bg-codex-wrapper.sh) で共有する EXIT trap セットアップ関数を
# 提供する。 wrapper (run-pre-push-codex-review.sh) は本 lib を使わない — 理由は下記。
#
# ## なぜ必要か
#
...
#   - block-pre-push-codex: fail-closed 設計の deny JSON を返せていない可能性
...
#
# ## なぜ wrapper (run-pre-push-codex-review.sh) は本 lib を使わないか
#
...
# error pattern をカバーする設計に倒している。 詳細根拠は run-pre-push-codex-review.sh
# ヘッダ参照。
#
# ## 呼び出し規約
#
# install_exit_trap <tag> <impact>
#   <tag>    : hook を特定する短いラベル (例: "block-pre-push-codex", "auto-mark")
#              stderr ログの `[pre-push-codex-review/<tag>]` プレフィクスに使われる
...
_pre_push_review_exit_handler_dispatch() {
  ...
  if [ "$exit_code" -ne 0 ]; then
    printf '[pre-push-codex-review/%s] 予期せぬエラーで hook が exit %s で終了しました。\n' \
      "$tag" "$exit_code" >&2
    printf '[pre-push-codex-review/%s] %s marketplace https://github.com/natsuume/natsuume-cc-marketplace に hook 実装の bug として報告してください。\n' \
      "$tag" "$impact" >&2
  fi
}

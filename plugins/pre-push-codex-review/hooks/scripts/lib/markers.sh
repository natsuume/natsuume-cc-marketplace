#!/bin/bash
# markers.sh
# pre-push-codex-review プラグインが扱うマーカーファイル名を単一ソース化する。
#
# block-pre-push-codex.sh が読み (codex マーカーのハッシュ検証)、 auto-mark.sh が final
# marker を書き込む。 run-pre-push-codex-review.sh は codex review 完了時の pending
# attestation を書き、 auto-mark.sh が parent-safe report の正常完了後に final marker へ
# 昇格する。 各 path が 1 文字でも乖離するとマーカーは永遠に一致せず push が通らなくなる
# 致命バグになるため、 ここに集約する。
#
# ## 本 plugin が扱うマーカー
#
# - CODEX_MARKER         ← codex review (OpenAI バグ検出 / wrapper script 経由) の final marker
...
# SendMessage による resume で SubagentStart が同一 agent_id で再発火する環境では、
# attestation の「作り直し」がフル review を経ないまま開始 hash を更新してしまう
# (= resume 後の再 stop が誤って marker を書ける経路)。 tombstone は「この agent_id は
# 一度 SubagentStop (stop_hook_active=false) まで到達した」 ことを恒久的に記録し、 同一
# agent_id での 2 回目以降の SubagentStart を構造的に拒否する。
...
marker_storage_dir() {
  ...
  if [ -z "$git_dir" ]; then
    printf '%s\n' '[pre-push-codex-review] git-dir が空のため marker path を解決できません。' >&2
    return 1
  fi
  printf '%s' "$git_dir"
}

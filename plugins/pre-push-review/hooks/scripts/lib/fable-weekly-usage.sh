...
# pre-push-review の reviewer 起動 model (fable / opus) を決める Fable 週次枠の使用率判定を
# 提供する。block-pre-push.sh (deny 文の起動案内) と bin/pre-push-review-reviewer-model
# (`/pre-push-review:review` が起動前に実行する判定コマンド) が source して共有する。
#
# ## 判定仕様
#
# agent-discipline の block-fable-subagent.sh と同じ判定を自前で実装する (plugin 間で実行時に
# コードを参照しない)。
...
#
# reviewer_model_decision は fable_weekly_usage の結果から、起動 model (REVIEWER_MODEL) と
# 利用者向けの判定理由 1 行 (REVIEWER_MODEL_REASON) を設定する。
...
# REVIEWER_MODEL / REVIEWER_MODEL_REASON は source した呼び出し側が読むグローバル変数。
...
reviewer_model_decision() {
  ...
  case "$FABLE_USAGE_STATUS" in
    available)
      ...
      REVIEWER_MODEL_REASON="Fable 週次枠の使用率 ${FABLE_USAGE_PERCENT}% が閾値 ${FABLE_USAGE_THRESHOLD}% 以下のため fable で起動します。"
      ;;
    over)
      REVIEWER_MODEL="opus"
      REVIEWER_MODEL_REASON="Fable 週次枠の使用率 ${FABLE_USAGE_PERCENT}% が閾値 ${FABLE_USAGE_THRESHOLD}% を超えているため opus で起動します。"
      ;;
    *)
      REVIEWER_MODEL="opus"
      REVIEWER_MODEL_REASON="Fable 週次枠の使用率を確認できないため opus で起動します (${FABLE_USAGE_DETAIL} 使用率 cache の producer は natsuume-statusline です)。"
      ;;
  esac
}

...
3. **Agent / Task tool**: `subagent_type: "pre-push-review:codex-reviewer"`、`model: "sonnet"`、 prompt: "codex review wrapper を foreground で 1 回起動し、 agent body の契約に従う parent-safe markdown report を返してください。実行可能な詳細を親 session に返さないでください。"、 description: "codex review wrapper の foreground 実行"

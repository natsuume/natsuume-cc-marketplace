...
- **Do not edit the wrapper or any other file, do not write to the pull request, and do not write the local record by hand.** This subagent's `tools` field grants `Bash, Read` — Edit / Write / Skill / Agent remain unavailable. The record is written by the wrapper and promoted by the plugin's lifecycle hook; a record produced by any other means is not evidence of an executed review.

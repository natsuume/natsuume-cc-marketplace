...
2. Run `gh pr view --json number,url,title,body,headRefOid,baseRefName,baseRefOid,closingIssuesReferences` for the current branch (failure: `pr-resolution`). The local HEAD must equal `headRefOid` (otherwise `head-mismatch`). The owner and name in `url` identify the pull request's repository.

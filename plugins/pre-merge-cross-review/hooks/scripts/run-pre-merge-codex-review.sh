...
#    を削除する (前回の中断が残した記録で merge が通る経路を残さないため)
...
# 7. PR 番号と head SHA を pending attestation へ書く (git-dir 直下。 GitHub には何も書かない)
#
...
#
# **本 wrapper は GitHub に何も書かない**。 レビュー完了の記録をローカルに書くところまでが
# 責務で、 subagent lifecycle hook (auto-mark.sh) が parent-safe report を検証して pending を
# final attestation へ昇格し、 merge gate (block-pre-merge.sh) が `gh pr merge` 時にその記録を
# 検証して merge を通す。 記録は merge の approve でも findings 0 件の証明でもない (findings が
# あっても 「レビュー済み」 として成立し、 findings への対応判断は review report を受け取った
# 親 session が行う)。 review report の本文は stdout に流すだけで、 ファイルには残さない。
#
# ## なぜ wrapper を介すか
#
...
# 含む) を放置すると、 merge gate がそれを受理して 「レビューしていない状態を通す」 経路に
# なるため、 レビューを始める前に必ず片付ける。 消せない場合は記録の一貫性を保証できないので
# 中断する。
rm -f "$FINAL_ATTESTATION_PATH" "$PENDING_ATTESTATION_PATH" || fail "古いレビュー記録 (${GIT_DIR} 直下) を削除できませんでした。 権限を確認してから再実行してください。"
...
# ローカル HEAD と PR head の一致確認。 不一致のままレビューすると、 記録する head SHA
# (= PR の head) と実際にレビューした内容が食い違うため実行しない。
...
# 異なる内容をレビューする」 乖離が生じ、 記録が実態を表さなくなる。
...
# 作業用ファイルに落としてから stdout へ流し、 stderr (進捗) はそのまま流す。
...
# レビュー記録を git-dir 直下に書く。 pending は git-dir 内の temp file から mv して atomic に
# 置く (読み手が中途半端な内容を観測しないため)。
PENDING_TMP=$(mktemp "${PENDING_ATTESTATION_PATH}.XXXXXX" 2>/dev/null) || fail "pending attestation の一時ファイルを作成できませんでした。 レビュー記録は残っていません。"
if ! printf 'pr=%s\nhead=%s\n' "$PR_NUMBER" "$HEAD_SHA" > "$PENDING_TMP" 2>/dev/null; then
  rm -f "$PENDING_TMP" 2>/dev/null || true
  fail "pending attestation を書き込めませんでした。 レビュー記録は残っていません。"
fi
if ! mv "$PENDING_TMP" "$PENDING_ATTESTATION_PATH" 2>/dev/null; then
  rm -f "$PENDING_TMP" 2>/dev/null || true
  fail "pending attestation を ${PENDING_ATTESTATION_PATH} へ配置できませんでした。 レビュー記録は残っていません。"
fi

note "レビュー記録をローカルに保存しました (pr=${PR_NUMBER} head=${HEAD_SHA})。 \`gh pr merge\` を実行すると merge gate が記録を検証してから merge に進みます。"

...
- [pre-push-codex-review](../pre-push-codex-review/): 会社環境向けの push 毎 codex review gate。本 plugin と同時に install しない前提です。review cadence の計数対象に `pre-merge-cross-review:codex-reviewer` の namespace を含みます (v4.0.0 以上) が、本 plugin 単独構成では review cadence 自体が適用されません

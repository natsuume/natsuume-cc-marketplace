...
| **防波堤層 (物理 deny)** | `PreToolUse` (matcher: `Agent\|Task`, `block-fable-subagent.sh`) | Agent / Task ツール呼び出し時 | (1) `tool_input.model` の fable 明示指定を無条件 deny。(2) model 未指定 (継承経路) は `CLAUDE_CODE_SUBAGENT_MODEL` が非 fable なら allow、env 不在/inherit 時は SessionStart が記録した session model state が fable の場合のみ deny。判定不能はすべて fail-open |
...
| `block-fable-subagent` | PreToolUse (`Agent\|Task`) | fable 明示指定と「model 未指定 + fable セッション継承」経路を deterministic な文字列判定で deny。LLM 評価型 hook は使わない (モデル可用性 SPOF の回避)。deny メッセージが sonnet / opus の明示を促すため、Claude は自己修正して再実行できる |

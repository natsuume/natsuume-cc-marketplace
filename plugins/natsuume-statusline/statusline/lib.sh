...
# トークン数を人間可読な短縮表記にする
# 1000未満はそのまま、1000以上は k / 999950以上は M を付け、小数1桁（末尾 .0 は省略）
# M の境界を 999950 とするのは、k の小数1桁丸めが 1000.0 になる値を "1000k" ではなく "1M" と表示するため
# 例: 512 -> "512", 75100 -> "75.1k", 200000 -> "200k", 999950 -> "1M", 1500000 -> "1.5M"
humanize_tokens() {
  local n="$1"

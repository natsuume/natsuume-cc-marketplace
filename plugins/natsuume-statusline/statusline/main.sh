...
# 入力が単一の JSON object でなければ空出力で終了する。不正 JSON・object 以外の JSON・
# 複数の JSON 値の連結・空入力を後続の jq に渡すと、パースエラーや型エラーが stderr に出るため。
# -s で入力ストリーム全体を配列に読み込み、要素がちょうど 1 つの object であることを検査する
# (-s なしの -e は最後の値しか見ないため、"[] {}" のような連結を通してしまう)。
printf '%s' "$input" | jq -e -s 'length == 1 and (.[0] | type == "object")' >/dev/null 2>&1 || exit 0

...
**横幅に合わせた段階的縮小** (2 行目・3 行目それぞれに独立して適用): 全内容が収まらない場合、`…` で切り詰める前に情報を保ったまま段階的に簡略化する。優先順位は ⓪ 使用率の小数を四捨五入して整数表示にする (`(45.2%)`→`(45%)`, `62.5%`→`63%`) → ① モデル名の末尾の括弧書きを削除 (`Opus 5.5 (1M context) (xhigh)`→`Opus 5.5 (xhigh)`。context サイズは ctx の最大トークンに残るので情報は保たれる。括弧書きが無いモデル名と 3 行目ではスキップ) → ② レートリミットのバー長を短縮 (最大 10 → 最小 3 文字) → ③ バーを削除 (`5h: 62% (58m)` のみ) → ④ ctx を使用率のみ `ctx: (45%)` にする (使用/最大トークンを削除。3 行目ではスキップ)。横幅に収まる最も豊かな表示を自動選択する。ctx の使用率はどの段階でも残す。それでも収まらない場合は、最後の手段として `…` で切り詰める

**描画幅**: 各行はターミナル幅 (`COLUMNS`) から 5 桁を差し引いた幅に収める。Claude Code は statusline をフッター領域 (左右の padding 2 桁ずつ + 右隣の通知領域との間隔 1 桁) に描画し、それを超えた右端を `…` で切り詰めるため。通知領域自体の幅や `statusLine.padding` 設定の分は考慮しない

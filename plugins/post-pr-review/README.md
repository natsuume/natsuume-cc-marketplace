...
これは強制ではなく **誘導** であり、Web UI などからの `gh pr ready` には影響しません。Claude Code 経由の PR 作成時のみ機能します。
...
4. git push
5. gh pr create --draft ... (git-guardrails が --draft を強制)
...
10. ユーザーが ready マーク (Claude Code 経由でも、Web からでも可)
```

## 既知の制約

- **強制ではなく誘導**: `additionalContext` で次の発話に注入するだけなので、Claude が無視することは原理的に可能です。強制したい場合は別途 `gh pr ready` をブロックする hook を組む必要があります。
...
- [git-guardrails](../git-guardrails/) — `gh pr create` 時に `--draft` を強制 / master への直接 push を禁止

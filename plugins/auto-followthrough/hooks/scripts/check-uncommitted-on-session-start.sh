...
2. 各ファイルを以下のいずれかに分類:
   ...
   - **(d) 不明** — ユーザに分類を依頼
3. 推奨アクションを 1〜数行でユーザに報告し同意を取る (例: 「X は (a) として commit に含めます」)。同意を得るまで git add / commit / stash / branch 切り出しは行わない

すべてが明らかに (a) かつ小規模なら「これらは今回の対象として進めます」の 1 行報告で足りる。auto-followthrough の commit→PR→merge フローは、この分類確定後に進めること。
EOF
)

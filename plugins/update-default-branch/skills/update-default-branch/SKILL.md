...
**削除結果の検証**: 一部のブランチだけ削除に失敗しても (worktree にチェックアウト中など)、`git branch -D` は残りのブランチの削除を続行し、非ゼロで終了します。終了コードだけでは「どれが削除済みか」がわからないため、出力を 1 行ずつ読み、削除対象ごとに `Deleted branch <name> (was <sha>).` の行があるかを照合してください。`error:` 行は `Deleted branch` 行より先に表示されることがあり、出力順は引数の並びと一致しません。`Deleted branch` 行が無いブランチは削除されていないので、ユーザーへの事後報告では削除済み (ブランチ名と SHA) と削除失敗 (ブランチ名と `error:` 行の内容) を分けて伝え、失敗したブランチは「トラブルシューティング」節で原因を確認します。出力から判断できない場合は手順 6 のコマンドを再実行し、ローカルに残っているブランチを確認してください。

> **注意 (`[gone]` の意味)**: `[gone]` は「リモート側の対応 ref が消えている」状態を指し、必ずしも「PR がマージ済み」とは限りません。リモートでの force-delete やリネームでも `[gone]` になります。`git branch -D` は merge 検査を skip するため、まれに「ローカルにのみ存在するコミットを保持する `[gone]` ブランチ」が誤削除される可能性があります。`git branch -D` の出力には削除前の SHA が表示されるので、削除結果 (ブランチ名と SHA) は必ずユーザーに事後報告してください。誤削除に気づいた場合は表示された SHA を `git switch -c '<name>' <sha>` で復活できます (約 30 日は `git reflog` でも遡れます)。マージ済み以外の理由で `[gone]` になる branch を残しておきたい場合は、本 Skill の手順 7 を実行する前に作業ブランチへ移しておくか、Skill の利用自体を控えてください。
...
`git branch -D` は worktree にチェックアウトされたブランチを削除できません。`git worktree list` で確認し、不要なら `git worktree remove <path>` してから再度削除してください。同じ呼び出しに列挙した他のブランチの削除は続行されているため、再削除の対象は手順 7 の「削除結果の検証」で `Deleted branch` 行が無かったブランチだけです。

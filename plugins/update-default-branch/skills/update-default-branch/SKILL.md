...
出力されたブランチ名を `ORIGINAL_BRANCH` として **会話コンテキストで記憶** します。手順 9 でこのブランチに戻すか、削除された場合は新しい作業ブランチを切るかの判断に使います。記憶した値は **ユーザー向けの応答文にも明記** してください (例: 「元のブランチ: feat/foo」)。途中で context が圧縮されても要約に残り、手順 9 で参照できます。
...
失敗した場合 (ローカルが分岐している) は削除フェーズ (手順 6〜8) に進まず **ここで中断** し、トラブルシューティング節に従ってユーザーに状況を報告します。中断前に **デフォルトブランチに居着かない** よう、**手順 9 と同じ分岐表に従って復帰** してください: `ORIGINAL_BRANCH` が空 (detached HEAD) / デフォルトブランチと同一 / 不明の場合は switch せずユーザーに次の作業ブランチを確認し、それ以外は以下で復帰します。
...
削除対象が複数ある場合も **1 回の呼び出しに列挙** します (該当がなければこの手順をスキップして手順 9 へ)。各ブランチ名は個別に single quote で囲みます。

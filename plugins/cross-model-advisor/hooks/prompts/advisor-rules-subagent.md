...
# cross-model-advisor: Codex への相談 (subagent)
...
- **実行**: 通常 subagent は Codex wrapper / companion を Bash で直接起動しない。相談プロンプト (タスク・証拠・質問 1 つ) を self-contained に組み立てて最終報告で親へ返し、`cross-model-advisor:codex-advisor-runner` の Agent (`model: "sonnet"` を明示し、起動 mode は指定しない) として実行するよう依頼する。PreToolUse gate は role 固有 runner 以外の直接起動を deny する
- **許可範囲**: 委任指示による cross-model-advisor の使用許可は、親が当該相談 request を `cross-model-advisor:codex-advisor-runner` へ渡すことを許可する。通常 subagent 自身による wrapper 実行、project への prompt 書き出し、その他の副作用操作は含まれない
